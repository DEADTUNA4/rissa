...
| `nci` | chemical DB `fff` | 32M | 1698K | 1698K | tie | `RAW` | **MDL cost dump 64K: RAW 3965, BWT 4525, FLOAT_SPLIT 5674 → RAW wins because transforms don't help, not threshold** |
...
| `NOAA binary sensor` 500K `gen_noaa` `fff` | 213K | 238K `SHUFFLE_8` tie | tie | `SHUFFLE_8` |
| `Yellow parquet` 2M ent 8.00 | 1996K | 1996K | tie at Shannon limit | `RAW` |
| `Columnar 3.6M` synthetic | 204K | **39K -80%** | **WIN** | `SHUFFLE_8/BIT_PLANE` |
| `Sensor 6M` synthetic | 1610K | **1038K -35%** | **WIN** | `SHUFFLE` |
| `5MB x*` | 892 | 924 | lose +32B | early-stop 10× 0.3→3.0 MB/s |

**Reframed target (was "structured/sensor/log columnar"):**

> **Actual evidence:** Wins on **bit-plane separable** data (16-bit medical `x-ray` via `BIT_PLANE`, synthetic columnar via `SHUFFLE`), ties on general structured/scientific (`nci`/`xml`/`sao` all `RAW`), small-file header loses on logs. **Truer claim:** `rissa` wins on bit-plane separable (images, certain sensor) and ties on general structured — invest in `SHUFFLE`/`FLOAT_SPLIT` stride detection for columnar, not `BWT` for 1M+ (still stubbed 256K-1M closed, 1M+ stubbed).
